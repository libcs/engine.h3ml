        ...
        static XNode CreateXNode(GumboNode node)
        {
            switch (node.type)
            {
                ...
                case GumboNodeType.GUMBO_NODE_TEMPLATE:
                    ...
                    var attributes = elementNode.GetAttributes().Select(x => new XAttribute(NativeUtf8Helper.StringFromNativeUtf8(x.name), NativeUtf8Helper.StringFromNativeUtf8(x.value)));
                    return new XElement(elementName, attributes, elementNode.GetChildren().Select(CreateXNode));
                case GumboNodeType.GUMBO_NODE_TEXT: return new XText(NativeUtf8Helper.StringFromNativeUtf8(((GumboTextNode)node).text.text));
                case GumboNodeType.GUMBO_NODE_CDATA: return new XCData(NativeUtf8Helper.StringFromNativeUtf8(((GumboTextNode)node).text.text));
                case GumboNodeType.GUMBO_NODE_COMMENT: return new XComment(NativeUtf8Helper.StringFromNativeUtf8(((GumboTextNode)node).text.text));
                case GumboNodeType.GUMBO_NODE_WHITESPACE: return new XText(NativeUtf8Helper.StringFromNativeUtf8(((GumboTextNode)node).text.text));
